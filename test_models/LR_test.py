...
lr_test_pred = best_model.predict(X_test)
...
kappa = cohen_kappa_score(y_test, lr_test_pred)
...
# 计算预测概率
lr_test_probs = best_model.predict_proba(X_test)[:, 1]

# probabilities from the logistic regression model
joblib.dump(lr_test_probs, '../processing data/lr_test_probs.joblib')
